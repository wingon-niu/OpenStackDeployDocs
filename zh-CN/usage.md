...
1. 请先为您的所有OpenStack集群服务器安装好操作系统，只支持 Ubuntu 14.04 LTS Server 版（版本号 14.04 或以上），按照一般方法安装即可，无特殊要求。
...
- 如果要使用Ceph做为glance、cinder、nova-compute的后端存储，则需要运行下面两条命令，以更改安装配置信息，在后面的安装过程中将使用第2个前端节点做为ceph-deploy admin node，所有的计算节点做为mon和osd，第1个计算节点做为mds。同时前提条件是最少有2个计算节点，而且所有计算节点与控制节点不能使用相同的服务器。（由于现在尚未在网页上面实现安装配置Ceph的功能，所以需要手动执行这个步骤。如果不使用Ceph，则不需要执行这个步骤。）
